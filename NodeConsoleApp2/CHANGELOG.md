...
### 变更
- **存档版本机制**:
    - 新增统一的当前游戏版本常量，用于标识本轮对话驱动后的可执行版本。
    - 存档写入时强制记录当前游戏版本号。
    - 读取存档时，若存档版本与当前游戏版本不一致，则直接删除本地存档并拒绝加载。
    - 数据源版本不一致时，同样直接删除本地存档并拒绝加载。
- **敌人行为设计文档**:
    - 重写 `design/enemy_design.md`，建立与当前 `DMG_ARMOR -> 溢出进 HP` 规则一致的敌人行为方案。
    - 明确普通敌人的默认攻击模型应为：随机选择玩家有效部位，并使用普通的部位护甲攻击技能。
    - 定义 `behaviorId + skillGroups` 的敌人行为数据结构，作为后续实现依据。
- **敌人设计文档拆分**:
    - 将“敌人模板/数值设计”和“敌人行为/AI 设计”拆分为两个独立文档。
    - `design/enemy_design.md` 现在只负责敌人主设计与模板规范。
    - 新增 `design/enemy_behavior_design.md` 负责敌人行为模式、技能选择与部位攻击方案。

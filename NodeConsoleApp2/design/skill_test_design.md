...
*   **技能树/矩阵显示 (Skill Tree/Matrix)**:
    *   **分组布局**: 界面应将技能库按设计文档中的 **7大流派** 进行分组显示，每个流派拥有独立的显示区域 (Secton/Tab)，组内技能按 **稀有度** (Common < Uncommon < Rare < Epic < Legendary) 升序排列。
        *   *近战系*: 重装猛攻 (Juggernaut), 铁壁守卫 (Guardian), 猩红剑术 (Duelist)
        *   *远程系*: 极速游侠 (Ranger), 精密狙击 (Sniper)
        *   *魔法系*: 元素毁灭 (Elementalist), 神圣祝祷 (Cleric)
    *   **视觉状态**: 
        *   *未学习*: 技能图标呈现灰色/半透明，仅显示基础轮廓。
        *   *已学习*: 技能图标恢复彩色，边框高亮显示。
        *   *稀有度标识*: 使用彩色边框或角标区分技能等级（普通-白/灰，稀有-绿，卓越-蓝，史诗-紫，传说-金）。
*   **详细信息展示 (Tooltip/Detail View)**:
    *   **触发方式**: 鼠标悬停或点击技能图标时，在侧边或浮层显示完整详情。
    *   **内容细节**:
        *   *基础参数*: 名称、AP消耗、冷却(CD)、目标类型（单体/群体/自身）。
        *   *效果详解*: 具体的伤害公式、治疗量、Buff/Debuff 描述（例如：“对目标造成 150% 攻击力的物理伤害，并施加 [流血] 2 回合”）。
        *   *标签*: 显示技能类型标签（如 [近战] [火焰] [控制]）。
*   **模拟获取 (3选1) (Draft Mode)**: 
    *   *交互流程*: 点击“模拟升级奖励”按钮，界面弹出模态框，展示 3 张 **技能卡牌**（从系统技能库中随机抽取未掌握的）。
    *   *卡牌样式*: 卡牌应包含完整的技能视觉元素（图标、稀有度边框、描述）。
    *   *决策*: 点击卡牌确认选择后，技能树中对应的灰色图标瞬间 **点亮**，模态框关闭。
*   **模拟释放 (Execution Test)**: 
    *   *操作*: 仅允许选中 **已点亮** 的技能。选中后点击“释放”按钮（或双击图标），对右侧 Dummy Enemy 执行 `Skill.execute()`。
    *   *反馈*: 日志输出详细的数值运算过程（基础伤害 -> 护甲减免 -> 最终伤害），右侧敌人 HP/护甲条实时扣除。
*   **重置与管理**: 
    *   按钮“重置 AP/CD”：一键回复玩家行动力，清空技能冷却。
    *   按钮“重置技能树”：遗忘所有技能，恢复到初始状态，用于反复测试获取流程。

...
*   **技能树/矩阵显示 (Skill Tree/Matrix)**:
    *   **布局模式**: 摒弃单纯的列表，采用 **技能树** 或 **网格矩阵** 形式展示所有可用技能（包括未学会的）。
        *   *点亮机制*: 玩家已学会的技能图标 **高亮/彩色显示**，未学会的技能图标 **灰度/半透明显示**。这种方式能直观展示技能收集进度和潜在的技能路线。
    *   **稀有度标识**: 通过图标边框颜色或角标明确区分技能稀有度（如：白色-普通 Common, 蓝色-稀有 Rare, 紫色-史诗 Epic, 金色-传说 Legendary）。

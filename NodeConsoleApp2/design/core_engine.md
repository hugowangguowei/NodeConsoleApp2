...
### 6.4 ???????????????Fail-Fast????Mock Fallback

`DataManagerV2.loadConfigs()` ?????ε?????????????????????**???????????????**????????????磺??? JSON ????????????????/404????ν?????????淶?????????????????????????????

- **Fail-fast**????????????????????? JSON??У???????????????????????
- **?????mock fallback**????????汾???????????? mock ????????????????? UI ???????"????"/"?????"?????????????λ????

**?????????? (Implementation Notes)**
- `loadConfigs()` ??????
  - `gameConfig` ???????????? `{}`
  - ???? `throw` ??????????????????
- `loadMockConfigs()` ??????ó???? `throw`????????????

**??????? (Recommended Debug Flow)**
1. ?????? Console ?в? `DataManagerV2.loadConfigs()` ????????? stack??
2. ???? Network ??? `assets/data/config.json` ?? `sources.*` ??????·??????????HTTP ?????? 200
3. ???????????? JSON ?????????? "" (????)

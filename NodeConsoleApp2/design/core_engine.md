...
### 6.2 存档版本失配策略（Strict Save Versioning）

为避免旧存档与当前实现的数据结构发生隐式兼容，运行时必须维护一个**当前游戏版本号**。

规则如下：

1. 当前游戏版本号由单一源码常量维护。
2. 每次由开发对话驱动并完成的功能修改，都必须同步递增该版本值。
3. `save_game` 存档内必须记录保存时的游戏版本号。
4. 读取存档时，若：
   - `save.version !== currentGameVersion`
   - 或数据源版本不一致
   则直接卸载（删除）该存档，并拒绝加载。

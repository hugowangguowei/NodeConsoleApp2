...
### 3.8 核心伤害结算规则（Canonical Damage Rules）

本节定义技能系统的核心伤害语义；该规则为技能设计主规范，运行时实现必须与此严格一致。

#### 3.8.1 默认攻击路径

- 大多数常规攻击技能应设计为 `DMG_ARMOR`。
- 默认攻击路径为：**先伤害目标部位护甲，再将溢出部分结算到 HP**。

#### 3.8.2 `DMG_ARMOR` 语义

- `DMG_ARMOR` 表示“对目标部位护甲造成伤害”。
- 若目标部位当前护甲大于 0：
  - 先扣减该部位护甲；
  - 若本次护甲伤害超过剩余护甲，则超出的部分继续作用到 HP。
- 若目标部位当前护甲等于 0：
  - 本次伤害直接作用到 HP。
- 若技能按部位命中，可先应用该部位弱点，再进行上述结算。

#### 3.8.3 `DMG_HP` 语义

- `DMG_HP` 表示“直接对 HP 造成伤害”。
- 该类型**不经过护甲吸收**。
- 该类型属于少数特殊技能使用的语义，不应再被当作普通攻击的通用标签。

#### 3.8.4 `PIERCE` 语义

- `PIERCE` 同样属于直伤 HP 的特殊伤害路径。
- 其区别在于：该标签强调“机制上绕过护甲”，而不是“默认伤害类型”。

#### 3.8.5 设计约束

- 普通近战/远程/按部位打击技能，优先建模为 `DMG_ARMOR`。
- 只有明确需要“直伤生命”的技能，才应使用 `DMG_HP` 或 `PIERCE`。
- 技能文档与数据配置不得再把 `DMG_HP` 当作“护甲吸收后再扣血”的旧式普通伤害语义。
...
### 5.0 运行时契约（Strict Runtime Contract）

为保证高内聚、低耦合，技能运行时执行器只读取新版技能数据结构，不再兼容旧字段。

#### 5.0.1 运行时唯一输入

- `skill.target`：技能级目标声明
- `skill.actions[]`：技能执行序列
- `skill.actions[].target.binding`：动作级目标绑定
- `skill.actions[].effect`：效果定义
...
若运行时仍依赖这些字段，应视为实现错误，而不是兼容需求。

#### 5.0.3 effectType 的运行时语义

- `DMG_ARMOR`
  - 先伤目标部位护甲，再将溢出部分作用到 HP。
- `DMG_HP`
  - 直接伤害 HP，不经过护甲吸收。
- `PIERCE`
  - 直接伤害 HP，不经过护甲吸收。
...
不在上述契约内的 binding 模式，不允许静默回退，应直接报错。

#### 5.0.5 失败策略

- 缺少 `target/actions/effect` 任一关键结构：直接报错。
- `effectType`、`amountType`、`binding.mode` 不受支持：直接报错。
- 部位技能缺少具体部位：直接报错。

该策略的目的，是在数据与实现不一致时尽早暴露问题，而不是通过旧逻辑回退维持表面可用。

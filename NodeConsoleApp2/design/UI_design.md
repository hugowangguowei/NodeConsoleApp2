...
**(1) 技能面板 (.skill-panel)**
*   **占比**: Flex 9。
*   **布局**: 建议采用混合布局，包含上方的 **待执行队列区** 和下方的 **技能选择与详情区**。

*   **待执行队列区 (.action-queue-wrapper)**
    *   **功能**: 可视化显示本回合已预设的技能序列。
    *   **组件**: 水平排列的技能 Slot，显示图标、顺序及 AP 消耗。支持点击取消。
...
在 UI 布局上，`skill-panel` 需重构为 **从左至右的三栏式布局** (`.skill-deck-layout`)，以满足复杂的部位选择需求：

1.  **待执行队列 (Action Matrix)** (`Left Column`)
    *   **布局**: 二维列表形式（或矩阵形式）。
    *   **展示**: 按照 **攻击部位 (Target Part)** 进行分类展示。例如：
        *   `Row 1: Head Target` - 放置针对头部的技能卡槽。
        *   `Row 2: Body/Torso Target` - 放置针对躯干的技能卡槽。
        *   `Row 3: Custom/Other` - 放置通用或自我增益技能。
    *   **占位符 (Placeholders)**: 每个分类下有预设的空卡槽 (`.slot-placeholder`)。当技能未放入时，显示为空槽；当技能选中时，对应的空槽会高亮提示可放入。

2.  **技能库 (Skill Pool)** (`Center Column`)
    *   **展示**: 网格化显示所有可用技能。
    *   **状态**:
        *   **Normal**: AP 足够，未进入冷却。
        *   **Disabled**: AP 不足（图标变灰，显示红字消耗）或 冷却中（显示遮罩）。
        *   **Selected**: 玩家点击了技能，技能处于选中态，等待玩家点击左侧的**待执行队列**中的目标位置进行确认。
...
**流程 A: 技能入列 (Enqueue)**
1.  **Select Skill**: 玩家点击中间栏 **技能库** 中的技能 `S`。
    *   UI 反馈: 技能 `S` 变更为 Selected 状态。
    *   **Slot Highlight**: 左侧 **待执行队列** 中，所有该技能 `S` 支持的目标部位对应的 **占位符 (.slot-placeholder)** 开始闪烁高亮。
        *   *例如：选中一个“仅能攻击头部”的狙击技能，左侧仅有 `Head` 行的空槽高亮。*
        *   *例如：选中一个“治疗术”，左侧 `Self/Ally` 行的空槽高亮。*
2.  **Confirm Target**: 玩家点击左侧高亮的某个 **占位符** `P`。
    *   数据更新: 调用 `Engine.input.addSkillToQueue(S, P.targetPart)`。

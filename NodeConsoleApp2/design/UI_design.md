...
1.  **待执行队列 (Action Matrix)** (`Left Column`)
    ...
    *   **占位符 (Placeholders)**: 平时显示为空槽 (`.slot-placeholder`)。当选中技能时，根据技能的目标类型（敌/我）及允许部位，仅高亮对应区域的对应行空槽。
...
1.  **Select Skill**: 玩家点击中间栏 **技能库** 中的技能 `S`。
    ...
    *   **Slot Highlight**: 系统判断技能 `S` 的目标倾向（Self/Enemy）及可用部位。
        *   **若为进攻技能**: **敌方队列 zone** 中对应部位行的空槽高亮闪烁。
        *   **若为增益技能**: **玩家队列 zone** 中对应部位行的空槽高亮闪烁。

...
#### 3.1.1 退出技能选择状态（页面任意空白点击退出）

为避免技能选择进入“粘滞状态”（点了技能后只能继续点槽位/换技能才能退出，容易误操作），规划阶段引入统一的退出机制。

**触发条件**：当存在已选中技能（`selectedSkill != null`）时。

**触发方式**：点击页面上除以下“有效交互区域”之外的任意空白处，自动退出技能选择状态。

**退出行为**：
- 取消 Skill Pool 中当前 `.skill-icon-button.active` 的高亮
- 清空当前选择（`selectedSkill = null`）
- 清除 Action Matrix 的可放置高亮（如 `highlight-valid`）

**不触发退出（白名单区域）**：
- 点击技能按钮：`.skill-icon-button`
- 点击槽位：`.slot-placeholder`
- 点击技能树 Overlay：`.overlay-backdrop` / `.overlay-panel`
- 点击系统 Modal：`.modal-backdrop` / `.modal-panel`

#### 3.1.2 槽位编辑模式（方案B：显式开启编辑，避免误触破坏）

为降低误触导致的“已规划槽位被删除/被替换”的成本，引入 **编辑模式 (Edit Mode)**。

**核心原则**：
- 当用户处于“放置技能”的选择循环（已选中某技能，即 `selectedSkill != null`）时，**不允许对已占用槽位产生任何修改**。
- 若要编辑（删除/调整）已占用的槽位，必须显式开启“编辑模式”。

**UI 入口**：
- 在技能面板底部 `skill-sort-bar` 增加切换按钮：`编辑模式：关/开`。

**交互规则**：
- **放置模式（编辑模式=关）**
  - 点击空槽：按“技能 -> 槽位”规则放置技能（行为不变）
  - 点击已占用槽：不执行删除/替换，仅提示“槽位已锁定，开启编辑模式后才可修改”
- **编辑模式（编辑模式=开）**
  - 点击已占用槽：执行取消/移除该槽位的占用（等价于对该 `slotKey` 执行 `unassign`）
  - 点击空槽：仍可放置（若同时选中技能）

> 注：该模式用于规划阶段的“严格验证机制”，确保技能 B 的放置循环不会隐式影响技能 A 的既有占用。

#### 3.1.3 Action Matrix 槽位内容显示（占位符/缩写）

为解决槽位内固定 `S` 缺乏区分度的问题，规划阶段对槽位内容显示做如下规范。

**目标**：在极小空间内提供“快速识别”，同时保持网格布局稳定，不让长文本挤爆槽位。

**槽位内显示优先级**：
1. 若技能数据存在 `skill.icon`：显示 `icon`。
2. 否则显示技能名称缩写（abbrev）：
   - 中文：优先取前 2 个字；不足 2 个则取 1 个字。
   - 英文/数字：取第一个 token 的前 2 个字符并大写（如 `Fireball` -> `FI`）。

**Hover/Tooltip**：
- 鼠标悬停在已占用槽位时，应能看到完整技能名（可用 `title` 实现）。

**样式约束**：
- 槽位文本需适配 1-2 字符，使用 `white-space: nowrap; overflow: hidden; text-overflow: ellipsis;` 保证稳定。

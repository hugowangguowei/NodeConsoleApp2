...
本文件描述 `buff_editor_v3` 的**彻底重做**方案：以 `assets/data/buffs_v2_1.json` 为唯一数据源，并以其 `meta.enums` 与规范化后的 `effects[].payload` 结构驱动 UI。目标是打造一个“面向数据规范、面向批量生产、面向强校验与重构”的 Buff 编辑器。

关联文档：

- `design/buff_design.md`：Buff 数据规范（Schema / Data Spec）。
- `assets/data/buffs_v2_1.json`：包含 `$schemaVersion`、`meta.enums`、`meta.fieldNotes`、以及 `buffs` 数据库。
...
1. 以数据规范驱动 UI：下拉选项、默认值、字段说明来自 `buffs_v2_1.json.meta`，避免 UI 内部硬编码枚举。

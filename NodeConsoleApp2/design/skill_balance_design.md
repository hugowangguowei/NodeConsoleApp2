...
1. **伤害类（Damage）**
   - `damage.hp`: 直接对 HP 造成伤害，不经过护甲吸收
   - `damage.armor`: 对某部位护甲造成伤害；若该部位护甲不足或为 0，溢出部分进入 HP
...
- `DMG_HP`
  - 直接对 HP 结算，不经过护甲吸收。
  - 若技能按部位选中目标，可先应用该部位弱点，再将结果直接结算到 HP。
- `DMG_ARMOR`
  - 先伤害目标部位护甲。
  - 若目标部位护甲为 0，或护甲不足以吸收本次伤害，则剩余伤害继续进入 HP。
  - 常规攻击技能应优先使用该类型；这是战斗系统的默认伤害路径。
- `PIERCE`
  - 直接对 HP 结算，不经过护甲吸收；语义上属于稀有的特殊直伤。
- `HEAL`
  - 恢复目标 HP，不能超过 `maxHp`。
- `ARMOR_ADD`
...
1. **默认攻击路径：`DMG_ARMOR`**
   - 先命中目标部位护甲。
   - 再将溢出部分结算到 HP。
2. **特殊直伤路径：`DMG_HP` / `PIERCE`**
   - 直接结算到 HP。
   - 不参与护甲吸收。

这意味着：

- “普通攻击”“多数攻击技能”“按部位打击”的默认设计，应优先建模为 `DMG_ARMOR`。
- `DMG_HP` 不应再被当作“普通伤害”的通用标签，而应保留给明确声明为直伤 HP 的少数技能。
   - 作用属性（What）
   - 数值类型（How）
   - 时间点（When）
   - 周期（How long）

...
* B1 DoT：`buff_poison`、`buff_bleed`、`buff_burn`
...
* C1 硬控：`buff_stun`、`buff_freeze`（`skipTurn`）
* C2 软控：`buff_slow`（speed -5）
* C3 技能/部位禁用：`buff_silence_limb`

> `buff_silence_limb` 目前只有描述没有 `effects/statModifiers`。在实现前需要补一条清晰规则：它是“禁止拥有某 tag 的技能”，还是“禁止选择某部位作为目标”。两种规则的引擎判定点不同。
...
* `buff_strength`、`buff_freeze` 的 `def +5` 等
...
* E1 护盾/吸收：`buff_shield`
* E2 免死：`buff_immortality_hp`
* E3 护甲不朽：`buff_immortality_armor`
* E4 复活：`buff_revive`、装备被动 `passive_phoenix`
...
| `buff_burn` | B1 DoT | `onTurnStart` 固定伤害 |
| `buff_stun` | C1 硬控 | `skipTurn` |
| `buff_freeze` | C1 硬控 + D 属性修正 | `skipTurn` + `def +5` |
| `buff_slow` | C2 软控（速度） | `speed -5` |
| `buff_shield` | A4 / E1 护盾吸收 | `onTakeDamage` -> `absorbDamage` |
| `debuff_weak` | A1 输出修正 | `damageDealtMult -0.2` |
| `buff_weakness` | A1 输出修正 | `damageDealtMult -0.15`（与上语义重叠） |
| `buff_vulnerable` | A2 承伤修正 | `damageTakenMult +0.2` |
| `buff_strength` | D 属性修正 | `atk +5` |
| `buff_silence_limb` | C3 禁用类（待明确规则） | 目前只有描述，无可执行字段 |
...
* 现状：两者都属于 A1 输出修正（降低造成伤害），且标签同为 `weakness`，差异仅在数值/持续回合。
* 风险：
  * UI/编辑器层很难解释两者差别；
...
* 现状：只有 `description`，无 `effects/statModifiers`，属于“文档/数据冗余占位”。
* 修正建议：
  * 要么补齐可执行字段（见 M2/M6），要么标注为 `tbd` 并从“已完成 Buff 清单”移出。

#### R7. “定义口径重复”：同一概念同时用 `def` 与 `armor` 表达（破甲相关）

* 现状：A3 已明确“护甲按部位”，但 `buff_armor_pen` 的示例与 `buffs.json` 仍在用 `def` 做破甲。
...
补充：当前 `assets/data/buffs.json` 的 `buff_armor_pen` 仍是 `MODIFY_STAT_TEMP` + `def`。为了避免概念混乱：

* 文档以“护甲按部位”为第一原则，破甲应当影响 `armorMitigationMult`（或上下文中的 `armorMitigationMult`），而不是 `def`。
* 现阶段允许 JSON 暂不改，但需要在实现 BuffSystem 时提供一个兼容层：
  * 若遇到 `stat=def` 且 Buff 语义为破甲：在日志中提示“旧字段口径（def）将迁移到 armorMitigationMult”。
